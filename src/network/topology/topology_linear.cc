...
void Topology::Linear(                     //
    ...
    LOGE_IF(!(node_link || switch_link), "The linear topology allows only one device type");
    for (size_t i = 0; i < total_num - 1; i++) {
        Link(p2p, metadata, nodes, switchs, node_link, switch_link, i, i + 1);
    }
}

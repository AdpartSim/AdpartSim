...
void Topology::Ring(                       //
    ...
    LOGE_IF(!(node_link || switch_link), "The ring topology allows only one device type");
    for (size_t i = 0; i < total_num; i++) {
        size_t dst = i + 1 >= total_num ? 0 : i + 1;
        Link(p2p, metadata, nodes, switchs, node_link, switch_link, i, dst);
    }
}
